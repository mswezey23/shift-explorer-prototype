...
const elementOccursXTimes = (selector, count) => () => element.all(by.css(selector)).count().then(
  ...
  if (rowCount > 0) {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].length; j++) {
        ...
        const selector = `${tableSelector} tr:nth-child(${i + 1}) td:nth-child(${j + 1}), 
          ${tableSelector} tr:nth-child(${i + 1}) th:nth-child(${j + 1})`;
        const elem = element(by.css(selector));
...
const nameToSelector = (elementName) => `.${elementName.replace(/\s+/g, '-')}`.replace(/\.(\d)/, '.\\3$1 ');
...
const checkTableContents = (tableName, data, rowCount, callback) => {
  ...
  browser.wait(elementOccursXTimes(`table.${selector} tbody tr`, rowCount), waitTime,
    `waiting for ${rowCount} instances of 'table.${selector} tbody tr'`);
  if (hasHeader) {
