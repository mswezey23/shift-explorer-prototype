...
function AbstractCandles(client) {
  ...
  this.sumTrades = function (results, cb) {
    let sum = _.map(results, (period) => ({
      ...
      shiftVolume: _.reduce(period, (memo, t) => (memo + parseFloat(t[self.candle.amount])), 0.0).toFixed(8),
      btcVolume: _.reduce(period, (memo, t) => (memo + (parseFloat(t[self.candle.amount]) * parseFloat(t[self.candle.price]))), 0.0)
        .toFixed(8),
