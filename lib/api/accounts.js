...
module.exports = function (app) {
  ...
  function Account() {
    this.validAddress = (address) => (typeof address === 'string' && address.match(/^[0-9]{1,21}[S|l]$/g));

    this.validPublicKey = (publicKey) => (typeof publicKey === 'string' && publicKey.match(/^([A-Fa-f0-9]{2}){32}$/g));

    this.getAccount = (address, cb) => request.get({
    ...
    this.getAccountByPublicKey = (publicKey, cb) => request.get({
